...
export class IconButtonComponent implements OnInit {
  ...
  iconPosition = input<IconButtonProps['iconPosition']>('left');
  iconSize = input<IconButtonProps['iconSize']>('medium');
  ...
  baseClasses =
    'font-bold focus-visible:outline-none flex items-center justify-center focus-visible:ring-2 focus-visible:ring-offset-2 active:translate-y-px disabled:pointer-events-none disabled:opacity-50';

  iconSpacingClasses: Record<IconButtonProps['iconPosition'], string> = {
    left: 'gap-2',
    right: 'gap-2',
  };

  iconSizeClasses: Record<IconButtonProps['iconSize'], string> = {
    small: 'h-4 w-4', // 16px
    medium: 'h-5 w-5', // 20px
    large: 'h-6 w-6', // 24px
  };
  ...
  impactClasses: Record<ButtonProps['tone'], Record<ButtonProps['impact'], string>> = {
    primary: {
      ...
      light: 'bg-white text-primary border border-border-1 hover:bg-primary/30 focus-visible:ring-primary',
  ...
  ngOnInit(): void {
    this.classes = cx(
      ...
      this.full() ? 'w-full' : '',
      this.iconSrc() ? this.iconSpacingClasses[this.iconPosition()] : '',
    );
  }
